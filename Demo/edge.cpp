...
void Edge::NodeUpdated(Node *updatedNode)
{
    if(updatedNode == startNode || updatedNode == endNode)
    {
        setLine(startNode->x(), startNode->y(), endNode->x(), endNode->y());
    }
}

Node* Edge::GetStartNode() const noexcept
{
    return startNode;
}

Node* Edge::GetEndNode() const noexcept
{
    return endNode;
}
